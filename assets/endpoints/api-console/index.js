...
//  REFRESH
	refresh : async function(r,p,c){

	   /* var user     = await r.models.user.findById(p[0]);
		var p       = {params:{token:p[2],user:user.email,application:p[1]}}
		var params  = {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)}
		var _res    = await fetch(r.accounts.path+"/refresh",params); */
		//c(await _res.json());
...
//	APPLICATIONS
	applications : {
        ...
        var params = {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(p)}
        var res = await fetch(r.accounts.path+"/applications/search",params);
				c(await res.json());
      }
		},
		get : {
...
//	IMAGES
	images : {
		get : async function(r,p,c){
			c(await r.models.images.findById(p));
		},
